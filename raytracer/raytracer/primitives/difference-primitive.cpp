...
namespace
{
	class BinaryDifferenceImplementation : public primitives::_private_::PrimitiveImplementation
	{
	public:
		BinaryDifferenceImplementation(Primitive child1, Primitive child2) : m_child1(child1), m_child2(child2)
		{
			//NOP
		}
		...
		std::vector<std::shared_ptr<Hit>> find_all_hits(const Ray& ray) const override
		{
			...
			while (i1 + 1 != hits1.end() || i2 + 1 != hits2.end())
			{
				...
				// We're inside the difference if we're inside both one and not the other
	...
	Primitive build_difference_tree(std::vector<Primitive>& children, int from, int to)
	{
		assert(from < to);
		...
		else
		{
			...
			auto left = build_difference_tree(children, from, middle);
			auto right = build_difference_tree(children, middle, to);

			return Primitive(std::make_shared<BinaryDifferenceImplementation>(left, right));
		}
	}
...
Primitive primitives::make_difference(Primitive child1, Primitive child2)
{
	return Primitive(std::make_shared<BinaryDifferenceImplementation>(child1, child2));
}