#include "demos/demoVanderhulstKasper.h"
...
namespace
{
    ...
    protected:
        ...
        raytracer::Primitive create_root(TimeStamp now) override
        {
            ...
            //Sphere 1
            auto x1 = animation::animate(-5.0, 5.0, 1_s);
            Primitive primitive1 = sphere();
            primitive1 = translate(Vector3D(x1(now), 1, 0), primitive1);
            primitive1 = decorate(material, primitive1);

            //Sphere 2
            auto x2 = animation::animate(5.0, -5.0, 1_s);
            Primitive primitive2 = sphere();
            primitive2 = translate(Vector3D(x2(now), -1, 0), primitive2);
...
void demos::demoVanderhulstKasper(std::shared_ptr<pipeline::Consumer<std::shared_ptr<Bitmap>>> output)
{
